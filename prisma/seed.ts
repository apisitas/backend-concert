...
async function main() {
    ...
    // // ----- FIXED RESERVATIONS FOR ALICE ONLY -----
    // const aliceConcerts = concerts.slice(0, 3); // pick first 3 concerts for Alice

    // for (const concert of aliceConcerts) {
    //     // Check if Alice already reserved this concert
    //     const existing = await prisma.reservation.findUnique({
    //         where: { userId_concertId: { userId: alice.id, concertId: concert.id } },
    //     });
    ...
    console.log('Fixed test user Alice ID:', alice.id);
}
